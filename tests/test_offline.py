...
def run_worker(fake, seconds=0.9, keyframe=0.25):
    ...
    try:
        ...
        sess._last_close = {}     # worker records a close timestamp here on teardown
...
print("\n-- worker paces to the modeled drain rate, not to a buffered sendall (rapid-slide lag fix) --")
# A small Adaptive frame fits inside SNDBUF, so sendall returns before the projector drains it.
# If the worker paced on that (near-zero) measured duration it would flood the projector's decode
# FIFO -> accumulating display lag. The modeled floor must bound the send cadence to the projector's
# real intake even when send_strips returns instantly. Removing the floor collapses the gaps to the
# worker's loop time (~ms), so this test fails loudly if the fix regresses.
def run_worker_rapid(seconds=1.0):
    A = fm.build_scans(slide(1)); B = fm.build_scans(slide(2))
    sess = fs.MirrorSession.__new__(fs.MirrorSession)
    sess.stop_event = threading.Event(); sess._slock = threading.Lock()
    sess._state = {"scans": A, "seq": 1}
    sess.min_interval = 0.0; sess.auto_reconnect = False; sess.log = lambda *a: None
    sess._last_close = {}     # worker records a close timestamp here on teardown
    fake = FakeSender(); sends = []
    fake.send_strips = lambda scans, idxs: sends.append(
        (time.time(), sum(len(scans[i][0]) for i in idxs)))
    sess.current = {fake.ip: fake}; sess.wanted = {fake.ip: True}
    sess.alive = {fake.ip: True}
    sess.stats = {fake.ip: {"sent": 0, "errors": 0, "drops": 0, "reconnects": 0}}
    stop = threading.Event()
    def churn():
        i = 0
        while not stop.is_set():
            with sess._slock:
                sess._state["scans"] = (B if i % 2 else A); sess._state["seq"] += 1
            i += 1; time.sleep(0.002)              # change content ~500x/s
    ct = threading.Thread(target=churn, daemon=True); ct.start()
    wt = threading.Thread(target=sess._worker, args=(fake.ip, fake), daemon=True); wt.start()
    time.sleep(seconds); sess.stop_event.set(); stop.set()
    wt.join(timeout=3); ct.join(timeout=1)
    return sends

_sd_sends = run_worker_rapid(1.0)
# gap before send k is delayed by the drain of the frame sent at k-1
_gaps_ok = sum(1 for k in range(1, len(_sd_sends))
               if _sd_sends[k][0] - _sd_sends[k-1][0]
                  >= 0.7 * fm.modeled_drain_s(_sd_sends[k-1][1]))
check("content changes every loop, so the worker keeps sending", len(_sd_sends) >= 4,
      "%d sends" % len(_sd_sends))
check("send cadence is bounded by the modeled drain, not the buffered sendall",
      len(_sd_sends) >= 4 and _gaps_ok >= (len(_sd_sends) - 1) * 0.7,
      "%d/%d gaps >= 0.7x modeled drain" % (_gaps_ok, len(_sd_sends) - 1))
check("...so a flood of tiny frames cannot pile up (well under the loop rate of ~500/s)",
      len(_sd_sends) <= 60, "%d sends in 1.0s" % len(_sd_sends))
...
# sends frames constantly, a slide sends one every ~10s). A hand-sent 1s beacon to a projector silenced it
...
# 2026-09-11, a projector log: "slow send (9) - backing off to 0.00s". The backoff is
