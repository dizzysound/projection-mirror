...
print("\n-- Power Off stops presenting first, then powers off (2026-09-12 fix) --")
class _FakeSessPO:
    def __init__(self, ips): self.ips = list(ips); self.wanted_calls = []
    def set_wanted(self, ips): self.wanted_calls.append(list(ips))
d = build(two)
A, B = "10.0.0.1", "10.0.0.2"
d.mirroring = True
d.mirror_session = _FakeSessPO([A, B])
_pj_calls = []
d._pj = lambda ips, *a, **k: _pj_calls.append((list(ips),) + a)
d._target_ips = lambda: [A]
d.powerOff_(None)
check("power off removes the target from the mirror first", d.mirror_session.wanted_calls == [[B]],
      d.mirror_session.wanted_calls)
check("power off still sends the PJLink power-off to the target", _pj_calls and _pj_calls[0][0] == [A],
      _pj_calls)
# powering off ALL mirrored projectors stops the whole mirror instead of leaving an empty session
d2 = build(two); d2.mirroring = True; d2.mirror_session = _FakeSessPO([A]); d2._pj = lambda *a, **k: None
d2._target_ips = lambda: [A]
_stopped = []; d2.stop_ = lambda s: _stopped.append(True)
d2.powerOff_(None)
check("powering off the last mirrored projector stops the mirror", _stopped == [True], _stopped)
# not mirroring: power off just sends the command, no session calls
d3 = build(two); d3.mirroring = False; d3.mirror_session = None
_pj3 = []; d3._pj = lambda ips, *a, **k: _pj3.append(list(ips)); d3._target_ips = lambda: [A]
d3.powerOff_(None)
check("power off when idle just sends the command", _pj3 == [[A]], _pj3)
