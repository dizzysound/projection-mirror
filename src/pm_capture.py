...
class DisplayStreamSource:
    ...
    def _run(self):
        ...
        # Frames are delivered on the serial dispatch queue created above, NOT through this thread's
        # run loop, so there is nothing here to pump. CFRunLoopRunInMode on an EMPTY mode (no sources
        # or timers are registered on this thread) returns kCFRunLoopRunFinished IMMEDIATELY instead
        # of blocking for the timeout, which turned this loop into a 100%-CPU spin. That spin held the
        # GIL and starved the encode/send threads, so slides stopped refreshing after a while. This
        # thread's only job is to keep the stream reference alive until stop, so just wait cheaply.
