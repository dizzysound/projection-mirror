...
MIN_INTERVAL = 0.15          # see pm_app.py: 0.08 wedged a projector on 2026-09-09 after ~90s
    ...
    def _worker(self, ip, s):
        ...
        while not self.stop_event.is_set() and self.wanted.get(ip, True):
            ...
            try:
                ...
                # sendall blocks until the projector accepts -> its duration is the drain rate ONLY
                # when the frame is bigger than the socket send buffer. A small Adaptive frame fits
                # inside SNDBUF, so sendall returns early and `dur` under-reports the real drain; the
                # modeled floor bounds the worker to the projector's true intake so slides can't pile
                # up in its decode FIFO (the rapid-slide-switch lag). No-op for large HQ frames.
                sent_bytes = sum(len(scans[i][0]) for i in idxs)
                prev = scans                      # only on success (v1.1 stale-band bug)
                if force_full: last_full = now
                ema = dur if ema is None else (0.7 * ema + 0.3 * dur)
                interval = max(self.min_interval, ema, fm.modeled_drain_s(sent_bytes))
                st["sent"] += 1
                st["bytes"] = st.get("bytes", 0) + sent_bytes
