...
Originally both projectors were driven from one loop. When one projector began timing out, its
blocking `sendall` stalled the other too, and the whole mirror froze. Now the producer publishes the latest
...
- **Pacing** — the interval is `max(min_interval, ema, modeled_drain)`. `min_interval` is the mode
  floor (0 s in the GUI's free-run, 0.15 s in the CLI). `ema` smooths the measured `sendall`
  duration, which equals the real drain rate *only when the frame is larger than the socket send
  buffer* — a large HQ frame blocks in `sendall`. A small Adaptive frame fits inside `SNDBUF`, so
  `sendall` returns before the projector has drained it and `ema` under-reports; without a floor the
  worker over-sends and slides pile up in the projector's decode FIFO (accumulating display lag on
  rapid slide switching). `modeled_drain` (`DRAIN_S_PER_BYTE·bytes + DRAIN_LATENCY_S`, ~1.9 MB/s +
  32 ms) is the empirical intake rate; flooring with it bounds the worker to what the projector can
  physically accept, and is a no-op for large frames where `ema` is already larger.
