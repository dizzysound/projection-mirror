...
## Standby mode — keep it "Normal", not ECO

Leave the projector's **Standby Mode set to Normal (networked), not ECO.** In Normal standby the
projector keeps its LAN module powered so it can be woken and driven over the network — which is what
this app relies on to associate and to answer PJLink at all. A fan running in standby is that
keep-alive, not a cooling cycle (cooling reads `%1POWR=2`; standby reads `%1POWR=0`) and not a fault.
ECO standby stops the fan but shuts the network module down, so the projector can no longer be woken
over the LAN — you must power it on with the remote before each use.
